...
@section('content')
<a href="{{ route('admin.apps.index') }}" class="text-sm text-teal-600 hover:underline">← Kembali</a>
<h1 class="text-2xl font-bold text-ink mt-2 mb-6">{{ $app ? 'Edit Aplikasi' : 'Tambah Aplikasi' }}</h1>

<form method="POST"
      action="{{ $app ? route('admin.apps.update', $app) : route('admin.apps.store') }}"
      class="max-w-xl bg-card rounded-xl shadow-sm border border-line p-6 space-y-4">
    @csrf
    ...
    <div>
        ...
        <textarea name="description" rows="3" required
                  class="w-full rounded-lg border border-line-strong px-3 py-2 text-sm">{{ old('description', $app->description ?? '') }}</textarea>
        @error('description') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
    </div>

    <div class="grid grid-cols-2 gap-4">
        <div>
            <label class="block text-sm font-medium text-ink mb-1">Harga / Bulan (Rp)</label>
        ...
        <div>
            ...
            <select name="status" class="w-full rounded-lg border border-line-strong px-3 py-2 text-sm">
    ...
    <div>
        <label class="block text-sm font-medium text-ink mb-1">Logo URL (opsional)</label>
