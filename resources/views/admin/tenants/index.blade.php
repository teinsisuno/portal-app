...
<div class="flex items-center justify-between mb-6">
    <h1 class="text-2xl font-bold text-ink">Kelola Tenant</h1>
    <div class="flex gap-3 text-sm">
        <a href="{{ route('admin.tenants.index') }}" class="text-teal-600 font-medium border-b-2 border-teal-600">Tenants</a>
...
<form method="GET" class="mb-6 flex gap-3">
    <input type="text" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Cari nama / slug / email"
           class="flex-1 rounded-lg border border-line-strong px-3 py-2 text-sm">
    <select name="status" class="rounded-lg border border-line-strong px-3 py-2 text-sm">
        <option value="">Semua status</option>
        @foreach (['pending', 'active', 'suspended'] as $s)
            <option value="{{ $s }}" @selected(($filters['status'] ?? '') === $s)>{{ ucfirst($s) }}</option>
        @endforeach
    </select>
    <button class="bg-teal-600 hover:bg-teal-700 text-white font-medium px-4 py-2 rounded-lg">Cari</button>
</form>
            ...
            @forelse ($tenants as $tenant)
                <tr>
                    <td class="px-6 py-3">
                        ...
                        <p class="text-xs text-muted">{{ $tenant->email }}</p>
                    </td>
                    <td class="px-6 py-3 text-muted font-mono text-xs">{{ $tenant->slug }}</td>
                    <td class="px-6 py-3">
                    ...
                    <td class="px-6 py-3 text-muted">{{ $tenant->subscriptions_count }}</td>
                    <td class="px-6 py-3">
                        <a href="{{ route('admin.tenants.show', $tenant) }}" class="text-teal-600 hover:underline">Detail</a>
                    </td>
                </tr>
            @empty
                <tr><td colspan="5" class="px-6 py-8 text-center text-muted">Tidak ada tenant.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
